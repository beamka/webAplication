...
public class UsersServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String result = "";
		List<Map<String, String>> dataOut = godb.SelectAllData();
		if (!dataOut.isEmpty()) {
			for (Map<String, String> row : dataOut) {
				result = result + "<label for=\"result\">" + row.get("firstName") + "</label>"
						+ "<label for=\"result\">" + row.get("lastName") + "</label>" + "</label>"
						+ "<label for=\"result\">" + row.get("birthday") + "</label>" + "<label for=\"result\">"
						+ row.get("phone") + "</label>" + "<label for=\"result\">" + row.get("email") + "</label>"
						+ "<label for=\"result\">" + row.get("country") + "</label>" + "<label for=\"result\">"
						+ row.get("city") + "</label>" + "<label for=\"result\">" + row.get("region") + "</label> <br>";
			}
		}
